  ...
  try {
    ...
    switch (answer) {
      ...
      case "research":
        ...
        await sendWebAppButtonWithMessage(
          bot,
          chatID,
    ...
    await checkIsUserReagentManager(reagentManagerChatID)
      ...
      .then((prompt) => {
        ...
        updateReagentApplications(prompt.data.appID, {
          ...
          sendNotification(bot, prompt.data.chatID, appConfirmation)
        );
      })
      .then(() =>
        sendNotification(bot, reagentManagerChatID, appConfirmationForManager)
      )
