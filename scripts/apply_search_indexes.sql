...
CREATE INDEX IF NOT EXISTS "idx_equipment_description_trgm" 
ON "Equipment" USING gin(description gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "idx_equipment_brand_trgm" 
ON "Equipment" USING gin(brand gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "idx_equipment_serial_number_trgm" 
ON "Equipment" USING gin("serialNumber" gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "idx_equipment_inventory_number_trgm" 
ON "Equipment" USING gin("inventoryNumber" gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "idx_equipment_category_trgm" 
ON "Equipment" USING gin(category gin_trgm_ops);

-- Связанные таблицы (через JOIN при поиске)
...
CREATE INDEX IF NOT EXISTS "idx_classification_name_trgm" 
ON "Classification" USING gin(name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "idx_measurement_name_trgm" 
ON "Measurement" USING gin(name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "idx_equipment_type_name_trgm" 
ON "EquipmentType" USING gin(name gin_trgm_ops);

CREATE INDEX IF NOT EXISTS "idx_equipment_kind_name_trgm" 
ON "EquipmentKind" USING gin(name gin_trgm_ops);